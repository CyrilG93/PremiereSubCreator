...
async function createUserInstaller(compilerPath, version, runtimeManifest, mode) {
  ...
  const iss = [
    ...
    "begin",
    "  { // Warn before Install because antivirus checks can briefly stop the wizard from repainting. }",
    "  Result := 'Click Install to continue with the installation.' + NewLine + NewLine +",
    "    'Important: after you click Install, this window may appear frozen for a few seconds while Windows checks the installation files. This is normal; please wait.';",
    "end;",
    "",
