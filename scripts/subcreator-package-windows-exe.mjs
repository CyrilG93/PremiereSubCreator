// // Build the complete Windows installer with its private runtime embedded.
...
const privateRuntimeVersion = process.env.SUBCREATOR_PRIVATE_RUNTIME_VERSION || "1";
...
async function stageFullPayload() {
  // // Stage the extension, README, and runtime installation script inside the complete EXE.
...
async function createFullInstaller(compilerPath, version) {
  // // Create the only supported Windows package with the runtime and base model embedded.
  const bundledBaseModelPath = await prepareBundledBaseModel();
  const outputBaseName = `SubCreator-v${version}-Windows-Full-Installer`;
  const outputPath = path.join(releasesDir, `${outputBaseName}.exe`);
  const scriptPath = path.join(installerRoot, "SubCreatorFull.iss");
  ...
  const iss = [
    ...
    `  Parameters := '-NoProfile -ExecutionPolicy Bypass -File "' + InstallerScriptPath + '" -PayloadRoot "' + PayloadPath + '" -RuntimeVersion "${escapePascalString(privateRuntimeVersion)}"';`,
    ...
    "    'Optional Whisper models can take several minutes to download.', nil);",
...
async function main() {
  // // Build the extension, validate the private runtime, and create the Full installer only.
